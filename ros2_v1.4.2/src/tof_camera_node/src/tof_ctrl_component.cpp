...
void TofCtrlNode::captureThread(void)
{
	...
	tof_camera_interface::msg::Notify	notify;
	krm::ExtTriggerType	trigger_mode = krm::EXT_TRG_STANDALONE;
	uint32_t	slave_wait_count_max = 86400U;
	uint32_t	slave_wait_count_current = 0U;

	if (cam_type_ != krm::PLAYBACK) {
		if (camera_->getProperty(krm::CMD_EXT_TRG_TYPE, &trigger_mode) != krm::SUCCESS) {
			RCLCPP_ERROR(this->get_logger(), "get Extension Trigger Type is failed");
		}
	}
	...
	while (loop) {
		...
		case krm::SUCCESS:
			...
			pre_time = cur_time;
			retry = 0;
			if (trigger_mode == krm::EXT_TRG_SLAVE) {
				slave_wait_count_current = 0U;
			}
		...
		case krm::ERR_TIMEOUT:
			if (trigger_mode == krm::EXT_TRG_SLAVE) {
				slave_wait_count_current++;
				if (slave_wait_count_current >= slave_wait_count_max) {
					RCLCPP_ERROR(this->get_logger(), "Slave wait count exceeded fixed maximum (%u)", slave_wait_count_max);
