...
void ColorTable::setColorRange(const Range& range)
{
	...
	if ((range_.min != range.min) || (range_.max != range.max)) {
		...
		std::array<Range, DIV_NUM>	color_range;
...
void ColorTable::createColorBar(const Range& d_range, uint16_t width, uint16_t height, std::vector<RgbaColor>& color_bar)
{
	std::lock_guard<std::mutex>		lock(mtx_);
	uint16_t x, y;
	RgbaColor* data;
